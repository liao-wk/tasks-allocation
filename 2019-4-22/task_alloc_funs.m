...
funs.Consumer_Resource = @Consumer_Resource; % 计算各个任务的资源消耗度
funs.Probability_tasks2 = @Probability_tasks2; % 计算两个交叉的时间窗的冲突概率
end
...
function [Conflict,conflict_withtasks] = Conflict_degree1(i,j,SE_sats,W,tasks_num)
m = size(SE_sats.TW_tasks{i,j},1); % 计算任务i在卫星j上有m个时间窗
Conflict = zeros(m,1);
conflict_withtasks = zeros(tasks_num,m); % 与任务i的冲突任务，任务i有m个时间窗，赋初始值为0
for p = 1:m % p代表任务i在卫星j上的第p个时间窗
    ...
    for q = 1:tasks_num
        ...
        if q == i || isempty(SE_sats.TW_tasks{q,j})
            % 对无法在卫星j上观测的任务(特例任务i)，在conflict_tasks中赋对应的权重为Inf
            conflict_withtasks(q,p) = inf; % 在conflict_tasks上填充冲突任务
            continue
        else
            ...
            for x = 1:k
                ...
                if tw(2)-tw(1)+tw_obj(2)-tw_obj(1)-(max([tw(2),tw_obj(2)])-...
                        min([tw(1),tw_obj(1)])) > 0
                    conflict_withtasks(q,p) = W(q)/k; % 在conflict_tasks上填充冲突任务
                    %  冲突度累加
                    Conflict(p,1) = Conflict(p,1) + W(q)/k;
                end
            end  
        end
    end
end
% 取任务i在卫星j上所有有效时间窗的最小冲突度,index1表示Conflict中最小的元素下标
[Conflict,index1] = min(Conflict);
% 按各个任务q与任务i的冲突度大小，对各个任务进行排序
[conflict_withtasks,index2] = sort(conflict_withtasks(:,index1)); % 按照冲突度的升序（从小到大）
conflict_withtasks = [index2,conflict_withtasks];
% 删除任务i,任务i与任务i不可能冲突.
for ii = 1:tasks_num
    if conflict_withtasks(ii,1) == i
        conflict_withtasks(ii,:) = []; 
        break
    end
end
end
%% 子函数6，计算无人机观测下任务的冲突度大小
function [Conflict,Conflict_withtasks] = Conflict_degree2(tasks_num,UAVs_N,UAVs_opp,D_UAV,base,t_pos,V_UAV,SE_tasks,duration,W)
Conflict = zeros(tasks_num,UAVs_N); % 各任务在各无人机下的冲突度矩阵
Conflict_withtasks = cell(tasks_num,UAVs_N);  % 各个任务在各资源下的冲突任务矩阵
% 最大距离冲突判断,如果不满足，进行最大时间窗判断
for i = 1:tasks_num
    if UAVs_opp(i,1) ~= 0
        % 任务i在无人机j的冲突度
        [Conflict(i,1),Conflict_withtasks{i,1}] = Judge_Dis_TW(i,tasks_num,...
            UAVs_opp,base,D_UAV,t_pos,V_UAV,SE_tasks,duration,W); 
        Conflict(i,:) = Conflict(i,1);
        for p = 2:UAVs_N
            Conflict_withtasks{i,p} = Conflict_withtasks{i,1};
        end
    end
end
% 将观测机会为0的位置上的元素替换成inf
Conflict(UAVs_opp==0) = inf; 
end
%% 子函数7，最大距离和时间窗判断,SE_tasks表示任务在无人机下的有效时间窗
function [conflict,conflict_withtasks] = Judge_Dis_TW(i,tasks_num,UAVs_opp,base,D_UAV,t_pos,V_UAV,SE_tasks,duration,W)
conflict = 0;
conflict_withtasks = zeros(tasks_num,1); % 与任务i的冲突任务，任务i只有1个时间窗，赋初始值为0
distance3 = distance_value(base,t_pos(i,:)); % 任务i与基地之间的距离
for p = 1:tasks_num
    if p == i || UAVs_opp(p,1) == 0
        conflict_withtasks(p,:) = inf; % 在conflict_tasks上填充冲突任务
        continue
    else
        ...
        duration2 = duration(p);
        % 最大距离判断（必定冲突）
        if distance > 2*D_UAV
            conflict = conflict + W(p);
            conflict_withtasks(p,:) = W(p); % 在conflict_tasks上填充冲突任务
        %  时间需求判断，（必定冲突判断）
        ...
        elseif SE_tasks(i,1) >= SE_tasks(p,2)
            ...
            elseif  SE_tasks(p,2)+time1+duration(p) > SE_tasks(i,1)
                duration(p) = duration(p)+time1;
                prob = Probability_tasks2(SE_tasks(i,:),SE_tasks(p,:),duration,i,p);
                conflict = conflict + W(p)*prob;
                conflict_withtasks(p,:) = W(p)*prob; % 在conflict_tasks上填充冲突任务
                duration(p) = duration2;
            end
        ...
        elseif SE_tasks(p,1) >= SE_tasks(i,2)
            if SE_tasks(i,1)+time1+duration(i) > SE_tasks(p,2)
                ...
                conflict_withtasks(p,:) = W(p); % 在conflict_tasks上填充冲突任务
            % （可能冲突判断），不相交的两个时间窗会存在任务冲突的情况
            elseif  SE_tasks(i,2)+time1+duration(i) > SE_tasks(p,1)
                duration(i) = duration(i)+time1;
                prob = Probability_tasks2(SE_tasks(i,:),SE_tasks(p,:),duration,i,p);
                conflict = conflict + W(p)*prob;
                conflict_withtasks(p,:) = W(p)*prob; % 在conflict_tasks上填充冲突任务
        ...
        %  可能冲突判断，根据两个任务的时间窗
        else % 对于持续时间，还需要考虑无人机从一个目标点到另一个目标点所花的时间
            duration(i) = duration(i)+time1;
            duration(p) = duration(p)+time1;
...
% 按各个任务q与任务i的冲突度大小，对各个任务进行排序
[conflict_withtasks,index2] = sort(conflict_withtasks(:,1)); % 按照冲突度的升序（从小到大）
conflict_withtasks = [index2,conflict_withtasks];
% 删除任务i,任务i与任务i不可能冲突.
for ii = 1:tasks_num
    if conflict_withtasks(ii,1) == i
...
tw(2) = tw(2) - duration(i);
tw_obj(2) = tw_obj(2) - duration(q);
...
%% 子函数11，计算各个任务的资源消耗度
function cons = Consumer_Resource(tasks_num,resource_num,opp,sel,base,t_pos,...
    duration,Dura,Rj,Rr,D_UAV,alpha,beta,gama)
cons = ones(tasks_num,resource_num);
for i = 1:tasks_num
    for j = 1:resource_num
        if opp(i,j) ~= 0
            if sel == 1 % 卫星观测
                ...
                cd = distance_value(base,t_pos(i,:));
            end
            cons(i,j) = (alpha*duration(i)/Dura+beta*Rj/Rr+gama*cd/D_UAV).^2;
        end
    end
end
end
%% 子函数12，计算两个不交叉的时间窗冲突的概率
function prob = Probability_tasks2(tw,tw_obj,duration,i,q)
tw(2) = tw(2) - duration(i);
tw_obj(2) = tw_obj(2) - duration(q);
% [T11,T12]是在[T21,T22]之后，且考虑持续时间的存在是导致冲突的发生，计算冲突可能性
delta1= tw(2)-tw_obj(2);
delta2 = tw(1)-tw_obj(1);
S0 = abs(tw(2)-tw(1))*abs(tw_obj(2)-tw_obj(1)); % 矩形的面积
if delta1*delta2 > 0
