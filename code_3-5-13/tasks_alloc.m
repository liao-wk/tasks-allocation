...
% 第2种启发式算法是将任务按照与子规划中心已分配的任务的互不干扰度最大原则，没有第1种好。
...
% for i = 1:subplan_num
%     if i == 1
%         [profit,T_schedule(i,:),T_unschedule(i,:)] = sats_func(solu2(i,:),sats,tasks,subplan,R_remain(1,2),R_remain(1,1),i);
%     elseif i == 2
%         [profit,T_schedule(i,:),T_unschedule(i,:)] = sats_func(solu2(i,:),sats,tasks,subplan,R_remain(1,2),R_remain(1,1),i);
%     elseif i == 3
...
best_far_fit = best_so_far_fit; % 储存历代best_so_far_fit
while T > 950
    for p = 1:L
        %% 邻域构造 
        ...
        %% 将未调度的任务按照最大互不干扰度(与已调度任务)的原则插入到对应的子规划中心
